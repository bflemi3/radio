define(['bluebird', 'underscore'], function(Promise, _) {
    ...
    // todo: go back to having a radio.start that will open a new window, store a reference to the new window and add __radio__ object to window. __radio__ on the new window will have a reference to the originating window.
    ...
    return {
        ...
        send: function(name, message) {
            ...
            var req = new Request(name, message);
            req.send();

            return _dispatch(name, message);
        }
    };
});